...
public:
	...
	LPCTSTR lpText, COLORREF txColor, UINT ItemType, UINT nRatioButton, UINT nRatioText) {
		...
		COLORREF utxColor = txColor ^ 0xFFFFFFFF;
		...
		DeleteObject(SelectObject(hMonoDC, hMonoBit));
		...
		if(nRatioText){
			DrawUtil::AlphaBlend(&textBuf, hBack, pt2, nRatioText);
		}
